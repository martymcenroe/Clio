...
> **Version:** 5.1.0
> **Last updated:** 2026-05-28 4:53:53 PM Central
...
> **Versioning:** semver per [AssemblyZero#1362](https://github.com/martymcenroe/AssemblyZero/issues/1362) principle 20 — major.minor.patch. See §20 change log for v1–v9 → semver retrofit.
> **Account-setup material:** moved to [`30004-cws-account-setup.md`](./30004-cws-account-setup.md)

## Clio CWS deployment state

Durable identifiers for the live Clio listing. Agent commands below (§0) reference these; updating any value here propagates to those commands.

| Item | Value |
|---|---|
| Extension ID | `lghjhpnjgfgekecamlbekpkbiaijmgbh` |
| Install URL | `https://chromewebstore.google.com/detail/clio/lghjhpnjgfgekecamlbekpkbiaijmgbh` |
| Dashboard listing | `https://chrome.google.com/webstore/devconsole` → Items → Clio |
| Publisher | ThriveTech.ai (`cto@thrivetech.ai`) |
| Submission tracking | [#95](https://github.com/martymcenroe/Clio/issues/95) |
...
| After CWS approves: run §15a — README + `docs/index.html` install-link update PR, tag the commit, comment + close #95 | `Run post-publish` (URL defaults to the install URL pinned in the deployment-state block; override with `Run post-publish <URL>` if needed) |
...
After Chrome approves the listing and emails the operator that the version is live, the operator says `Run post-publish` to the agent. The URL argument is optional — agent uses the pinned install URL from the deployment-state block at the top of this runbook unless the operator overrides with `Run post-publish <URL>`.

### 15a. Agent does (`Run post-publish` or `Run post-publish <URL>`)

1. Update the README install link from "Coming soon" / dev-mode instructions to a direct CWS install link at the pinned install URL — branch, commit, PR, merge
2. Update `docs/index.html` install link (`<p class="install-note">Coming soon to the Chrome Web Store.</p>` at line ~147) to a CWS install button at the pinned install URL — branch, commit, PR, merge
3. Tag the released commit: `git tag vX.Y.Z-published && git push origin vX.Y.Z-published` (X.Y.Z from `extensions/manifest.json`'s `version`)
4. Comment on #95 with the approval date and the pinned install URL; close the issue
5. Update `docs/releases/chrome-vX.Y.Z.md` — fill the `Submission date: TBD` line with the §14.3 submission timestamp and add a new line `Approval date:` with the date Chrome notified the operator
