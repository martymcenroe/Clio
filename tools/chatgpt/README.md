...
| `probe-reasoning.js` | is reasoning content reachable at all? Finds the affordance by **duration** (`for 5m 34s` — the one thing every wording has carried), by verb, and by attribute, then dumps its ancestor chain and measures text length **before and after clicking**, which is what decides whether a selector alone is enough or the capture has to click. Also prints a **backstop** listing of every unmatched clickable inside an assistant turn, so a wording nobody predicted cannot vanish: the first version matched verbs only and would have reported "nothing found" against a live `Worked for 5m 34s` (#321). Discovery logic lives in `reasoning-dom.js` so jsdom can exercise it (`tests/reasoning-dom.test.js`). Written for #316, where 5,644 captured messages all carry an empty `thinking` field. |
